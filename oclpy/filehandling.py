...
def load_range(path: Union[str, Path]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    if isinstance(path, str):
        path = Path(path)
    df = pd.read_csv(path, comment="#", header=None, delim_whitespace=True, names=['E', 'R', 'std'])
    return df['R'].values, df['std'].values, df['E'].values
