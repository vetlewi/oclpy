...
def get_version_info():
    ...
    elif os.path.exists('oclpy/version.py'):
        # must be a source distribution, use existing version file
        try:
            from ompy.version import git_revision as GIT_REVISION
        except ImportError:
            raise ImportError("Unable to import git_revision. Try removing "
                              "oclpy/version.py and the build directory "
                              "before building.")
    else:
        GIT_REVISION = "Unknown"
...
def write_version_py(filename='oclpy/version.py'):
    cnt = """
# THIS FILE IS GENERATED FROM OCLPY SETUP.PY
#
...
if openmp is None and platform.system() == 'Darwin':  # Check if macOS
    ...
    raise ValueError("Env var ompy_OpenMP must be either True or False "
                     ...
                     f"Now it is: {openmp}")
fname = "oclpy/matplib.c"  # otherwise it may not recompile
...
ext_modules = [
        ...
        Extension("oclpy.rebin", ["oclpy/rebin.pyx"],
                  # on MacOS the clang compiler pretends not to support OpenMP, but in fact it does so
                  extra_compile_args=extra_compile_args,
                  extra_link_args=extra_link_args,
                  include_dirs=[numpy.get_include()]
                  ),
        Extension("oclpy.gauss_smoothing", ["oclpy/gauss_smoothing.pyx"],
                  # on MacOS the clang compiler pretends not to support OpenMP, but in fact it does so
                  extra_compile_args=extra_compile_args,
                  extra_link_args=extra_link_args,
                  include_dirs=[numpy.get_include()]
...
setup(name='OCLpy',
      ...
      author_email=("jorgenem@gmail.com, "
                    ...
                    "erlenlim@fys.uio.no"),
      url="https://github.com/oslocyclotronlab/oclpy",
